...
'''
from app import app
from app.utils import ResponseUtil, RequestUtil, HookDataParse, JsonUtil
...
#     data = WebhookData.gitosc
    try:
        ...
        if webhook:
            ...
            if HookDataParse.get_repo_name(data) == repo and HookDataParse.get_repo_branch(data) == branch:
                # start to process, add history into database
                # waiting to done
                history = History(status='1', webhook_id=webhook.id, data=JsonUtil.object_2_json(data))
                history.save()
                # status is waiting
